...
class GBPForexEnvironment(gym.Env):
        def __init__(self,data:dict,*args,temporal_window=912,account_balance=100.00,risk_size=0.3,currency_spread=0.0,**kwargs) -> None:
                ...
                self.features_fields=[i for i in self.currencies.columns.to_list() if i!="Datetime" and i!="Date"]
                ...
                self.stop_loss_space=gym.spaces.Box(low=0,high=float(self.average_profit_pips*2))
                ...
                self.take_profit_space=gym.spaces.Box(high=float(self.average_profit_pips*2),low=10)
                # Account Spaces
                ...
                self.observation_space=gym.spaces.Dict({"temporal_window_state":gym.spaces.Box(low=float(self.min_value),
                                                                                       high=float(self.max_value),
                                                                                       shape=(self.temporal_window,
        ...
        def reset(self,*args,**kwargs):
                ...
                self.open_trades=[]
                self.trade_index=0
                self.current_price=self.temporal_window
                ...
                reset_obs={"temporal_window_state":np.array((self.currencies.iloc[self.current_price-self.temporal_window:self.current_price][self.features_fields].to_numpy()),dtype=np.float64),
        ...
        def step(self,action):
                ...
                state={"temporal_window_state":np.array(self.currencies.iloc[self.current_price-self.temporal_window:self.current_price][self.features_fields].to_numpy(),dtype=np.float64),
                                                                                       "balance":np.array([float(self.account_balance)],dtype=np.float64),
                                                                                       "equity":np.array([float(self.account_equity)],dtype=np.float64),
                                                                                       "margin":np.array([float(self.account_balance)],dtype=np.float64),
                                                                                       "spread":np.array([float(self.currency_spread)],dtype=np.float64),
                                                                                       "profit":np.array([float(self.total_profits)],dtype=np.float64),
                                                                                        "loss":np.array([float(self.total_losses)],dtype=np.float64)
                                                                                       }
...
class FXTorchEnv(EnvBase):
        ...
        def __init__(self,data,temporal_window=912,account_balance=100_000,risk_size=0.3,device="cpu",*args,**kwargs):
                ...
                self.risk_reward_ratio=2.0
                self.open_trades=[]
                self.currency_spread=0
                self.max_trades=10
                self.trade_index=0
                self.average_profit_pips=3000
                self.average_loss_pips=1500
                self.track_data={
                                "profits":[],
                                 "balance":[],
                                 "losses":[],
        ...
        def _reset(self,tensordict:TensorDict):
                ...
                reset_obs= TensorDict(
                        {"temporal_window_state":torch.tensor((self.currencies.iloc[self.current_price-self.temporal_window:self.current_price][self.features_fields].to_numpy()),dtype=torch.float64).view(1,self.temporal_window,len(self.features_fields)),
        ...
        def _step(self,state:TensorDict):
                ...
                TakeProfit=action[1].item()
                StopLoss=action[2].item()
                OptionFloat=action[0]
                Option=int(round(OptionFloat.item(),0))
                
                LotSize=action[3].item()
                ModificationId=action[4].item()
        ...
        def make_spec(self):
                ...
                num_columns=len(self.currencies.columns.to_list())
                self.observation_spec = CompositeSpec(
                        temporal_window_state=BoundedTensorSpec(
                                low=-torch.inf,
                                high=torch.inf,
                                shape=(1,self.temporal_window,len(self.features_fields)),dtype=torch.float64
                        ),
                        balance=BoundedTensorSpec(
                                low=-10000,
                                high=torch.inf,
                                shape=(1,),
                                dtype=torch.float64,
                        ),
                        equity=BoundedTensorSpec(
                                low=-100,
                                high=torch.inf,
                                shape=(1,),
                                dtype=torch.float64,
                        ),
                        # reward=UnboundedContinuousTensorSpec(shape=(1,),dtype=torch.float64),
                        # done=UnboundedContinuousTensorSpec(shape=(1,),dtype=torch.bool)
                        # ,
                ...
                self.reward_spec = UnboundedContinuousTensorSpec(shape=(1,),dtype=torch.float64)
